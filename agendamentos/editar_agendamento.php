...
<body>
    <div class="container mt-5">
        ...
        if (isset($_GET["id"])) {
            ...
            $sql_agendamento = "SELECT id_agendamento, id_paciente, id_dentista, data_agendamento, status FROM Agendamento WHERE id_agendamento = ?";
            $stmt_agendamento = $conexao->prepare($sql_agendamento);
            
            if ($stmt_agendamento === false) {
                echo "<p class='alert alert-danger'>Erro ao preparar a consulta de agendamento: " . htmlspecialchars($conexao->error) . "</p>";
            } else {
                $stmt_agendamento->bind_param("i", $id);
                $stmt_agendamento->execute();
                $resultado_agendamento = $stmt_agendamento->get_result();

                if ($resultado_agendamento->num_rows == 1) {
                    $agendamento = $resultado_agendamento->fetch_assoc();
                    ?>
                    <form action="atualizar_agendamento.php" method="POST">
                        <input type="hidden" name="id_agendamento" value="<?php echo htmlspecialchars($agendamento['id_agendamento']); ?>">
                            ...
                            <select class="form-control" name="id_paciente" id="id_paciente" required>
                                ...
                                if ($resultado_pacientes->num_rows > 0) {
                                    while ($row_paciente = $resultado_pacientes->fetch_assoc()) {
                                        $selected = ($row_paciente["id_paciente"] == $agendamento["id_paciente"]) ? "selected" : "";
                                        echo "<option value='" . htmlspecialchars($row_paciente["id_paciente"]) . "' " . $selected . ">" . htmlspecialchars($row_paciente["nome"]) . "</option>";
                                    }
                                } else {
                            ...
                            <select class="form-control" name="id_dentista" id="id_dentista" required>
                                ...
                                if ($resultado_dentistas->num_rows > 0) {
                                    while ($row_dentista = $resultado_dentistas->fetch_assoc()) {
                                        $selected = ($row_dentista["id_dentista"] == $agendamento["id_dentista"]) ? "selected" : "";
                                        echo "<option value='" . htmlspecialchars($row_dentista["id_dentista"]) . "' " . $selected . ">" . htmlspecialchars($row_dentista["nome"]) . "</option>";
                                    }
                                } else {
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control" id="data_agendamento" name="data_agendamento" value="<?php echo htmlspecialchars(str_replace(' ', 'T', $agendamento['data_agendamento'])); ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="status">Status:</label>
                            <select class="form-control" name="status" id="status">
                                <option value="Agendado" <?php if ($agendamento['status'] == 'Agendado') echo 'selected'; ?>>Agendado</option>
                                <option value="Cancelado" <?php if ($agendamento['status'] == 'Cancelado') echo 'selected'; ?>>Cancelado</option>
                                <option value="Concluído" <?php if ($agendamento['status'] == 'Concluído') echo 'selected'; ?>>Concluído</option>
                            </select>
                        </div>
                        
                        <div class="d-flex justify-content-between mt-3">
                            <button type="submit" class="btn btn-primary">Atualizar</button>
                            <a href="listar_agendamento.php" class="btn btn-link">Voltar para a Listagem</a>
                        </div>
                    </form>
                    <?php
                } else {
                    echo "<p class='alert alert-warning'>Agendamento não encontrado.</p>";
                }
                $stmt_agendamento->close();
            }
            $conexao->close(); // Fecha a conexão após todas as operações
        } else {
            echo "<p class='alert alert-danger'>ID do agendamento não especificado.</p>";
        }
        ?>
    </div>
